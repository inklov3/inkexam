package top.inklov3.auth.config;
...
public class SwaggerConfig {
    ...
    public GroupedOpenApi userApi() {
        String[] paths = {"/**"};
        String[] packagedToMatch = {"top.inklov3.auth.controller"};
        return GroupedOpenApi.builder().group("用户校验模块")
                .pathsToMatch(paths)
    ...
    public OpenAPI customOpenAPI() {
        return new OpenAPI()
                .info(new Info()
                        .title("inkexam-auth")
                        .version("alpha-1")
                        .description("考试系统用户验证模块")
                        .license(new License().name("Apache 2.0")
                                .url("https://exam.inklov3.top"))
